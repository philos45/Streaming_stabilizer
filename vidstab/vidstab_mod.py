...
class VidStab:
    ...
    def _gen_next_raw_transform(self):
        ...
        # print(self.prev_gray, self.prev_kps)
        # print(np.shape(current_frame_gray))
    ...
    def _process_first_frame(self, array=None):
        # read first frame
        _, _, _ = self.read_frame(array=array, pop_ind=False)
    ...
    def _init_trajectory(self, smoothing_window, max_frames, gen_all=False, show_progress=False):
        ...
        while True:
            # read current frame
            _, _, break_flag = self.read_frame(pop_ind=False)
            # print(break_flag)
    ...
    def read_frame(self, pop_ind=True, array=None):
        ...
        #     frame = array
        # self.grabbed_frame,frame = self.source.read()
        frame = self.frame_queue.source.read()[1]
        return self.frame_queue._append_frame(frame, pop_ind)
    ...
    def _apply_transforms(self, output_path, max_frames, use_stored_transforms,
        ...
        while True:
            ...
            i, frame_i, break_flag = self.read_frame()
            ...
            break_playback = playback_video(transformed, playback,
    ...
    #         _, frame_i, _ = self.read_frame(array=input_frame, pop_ind=False)
    #         if not use_stored_transforms:
    #             self._gen_next_raw_transform()
    ...
    def stabilize(self, input_path, output_path, smoothing_window=30, max_frames=float('inf'),
        ...
        #여기
        self.frame_queue.source = cv2.VideoCapture(input_path)

        print(self.frame_queue.source)
        print(np.shape(self.frame_queue.source))

        # print(cv2.VideoCapture(input_path))
        print(np.shape(cv2.VideoCapture(input_path)))
        ...
        self.frame_queue.max_len = 3
        self.frame_queue.max_frames = float('inf')
        self.frame_queue.frames = PopDeque(maxlen=3)
        self.frame_queue.inds =PopDeque(maxlen=3)
        self.frame_queue.i = None
        ...
        #                                           show_progress)
            ...
            self._init_trajectory(smoothing_window, max_frames, show_progress=show_progress)
            # self.frame_queue.populate_queue(smoothing_window)

        if self.auto_border_flag:
            frame_1 = self.frame_queue.frames[0]
            self.extreme_frame_corners = extreme_corners(frame_1.image, self.transforms)
            border_size = min_auto_border_size(self.extreme_frame_corners)
        ...
        self._apply_transforms(output_path, max_frames, use_stored_transforms=use_stored_transforms,
                               ...
                               output_fourcc=output_fourcc, progress_bar=None)
